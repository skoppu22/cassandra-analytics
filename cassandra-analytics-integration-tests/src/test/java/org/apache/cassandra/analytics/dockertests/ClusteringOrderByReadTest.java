...
/**
 * Port of {@code dockertests/tests/sbr/test_clustering_orderby.py}: verifies the bulk reader
 * correctly handles {@code WITH CLUSTERING ORDER BY (b DESC)} and does not return duplicate rows
 * when the same clustering key is written in a later SSTable — the later write must overwrite.
 */
class ClusteringOrderByReadTest extends DockertestBase
{
